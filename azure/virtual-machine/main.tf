...
resource "azurerm_network_security_group" "vm-network-security-group" {
  resource_group_name = var.resource-group-properties.name
  location            = var.resource-group-properties.location
...
resource "azurerm_network_security_rule" "vm-network-security-rule" {
  resource_group_name = var.resource-group-properties.name

  network_security_group_name = azurerm_network_security_group.vm-network-security-group.name
  ...
  depends_on = [
    azurerm_network_security_group.vm-network-security-group
  ]
}
...
resource "azurerm_network_interface_security_group_association" "vm-nsg-association" {
  ...
  network_security_group_id = azurerm_network_security_group.vm-network-security-group.id
...
resource "azurerm_linux_virtual_machine" "vm-linux-virtual-machine" {
  resource_group_name = var.resource-group-properties.name
  location            = var.resource-group-properties.location
  ...
  source_image_reference {
    publisher = var.virtual-machine-properties.linux-vm.source-image-reference-publisher
    offer     = var.virtual-machine-properties.linux-vm.source-image-reference-offer
    sku       = var.virtual-machine-properties.linux-vm.source-image-reference-sku
    version   = var.virtual-machine-properties.linux-vm.source-image-reference-version
  }
